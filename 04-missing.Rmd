...
As can be seen, most of the ("\N") value are in the columns mentioned above and the number of them are almost same. 

To investigate the distribution of ("\N") values values by rows:

```{r,echo=FALSE}
rowSums(is.na(resultsMissing)) %>%
  sort(decreasing = TRUE)

```

As can be seen for most of the rows, we have seven ("\N") values, since time represent in two columns ("Time" and "milisecond").
